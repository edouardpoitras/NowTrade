...
class Action:
    ...
    def raw(self): return NO_ACTION

class Long(Action):
    """
    Long Action represents going long in the market.
    """
    def __init__(self): Action.__init__(self, 'long')
    def raw(self): return LONG

class LongExit(Action):
    """
    Long Exit Action represents exiting a long position from the market.
    """
    def __init__(self): Action.__init__(self, 'longexit')
    def __eq__(self, other):
        other = str(other).replace(' ', '').replace('_', '').replace('-', '').lower()
        return other == 'longexit' or other == 'exitlong'
    def raw(self): return LONG_EXIT

class ExitLong(LongExit): pass

class Short(Action):
    """
    Short Action represents shorting in the market.
    """
    def __init__(self): Action.__init__(self, 'short')
    def raw(self): return SHORT

class ShortExit(Action):
    """
    Short Exit Action represents exiting a short position from the market.
    """
    def __init__(self): Action.__init__(self, 'shortexit')
    def __eq__(self, other):
        other = str(other).replace(' ', '').replace('_', '').replace('-', '').lower()
        return other == 'shortexit' or other == 'exitshort'
    def raw(self): return SHORT_EXIT

class ExitShort(ShortExit): pass
