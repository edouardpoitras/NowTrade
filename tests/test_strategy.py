...
class TestStrategy(unittest.TestCase):
    ...
    def test_simple_strategy(self):
        ...
        self.assertEqual(strat.realtime_data_frame.iloc[3]['PL_MSFT'], report_overview['net_profit'])
        ...
        self.assertEqual(strat.realtime_data_frame.iloc[3]['STATUS_MSFT'], 0)
    ...
    def test_trailing_stop_long_strategy(self):
        ...
        report_overview = strat.report.overview()
        self.assertTrue(np.isnan(strat.realtime_data_frame.iloc[0]['PL_PERCENT_MSFT']))
        self.assertEqual(strat.realtime_data_frame.iloc[-5]['PL_VALUE_MSFT'], -0.26999999999999957)
        self.assertEqual(strat.realtime_data_frame.iloc[1]['PL_VALUE_MSFT'], 0.40000000000000213)
    ...
    def test_trailing_stop_short_strategy(self):
        return
        enter_crit = criteria.Position(self.symbol.close, 'above', 25.88)
        exit_crit = criteria.TrailingStop(self.symbol, 0.45, short=True)
        ...
        self.assertTrue(np.isnan(strat.realtime_data_frame.iloc[0]['PL_PERCENT_MSFT']))
        self.assertTrue(np.isnan(strat.realtime_data_frame.iloc[-4]['PL_VALUE_MSFT']))
        self.assertEqual(strat.realtime_data_frame.iloc[-5]['PL_VALUE_MSFT'], 0.0400000000000027)
        self.assertEqual(strat.realtime_data_frame.iloc[1]['PL_VALUE_MSFT'], 0.40000000000000213)
        self.assertEqual(strat.realtime_data_frame.iloc[2]['PL_MSFT'], -94.080000000000837)
        self.assertEqual(strat.realtime_data_frame.iloc[3]['PL_PERCENT_MSFT'], 0.0015349194167308617)
        self.assertEqual(strat.realtime_data_frame.iloc[0]['ACTIONS_MSFT'], 0)
        self.assertEqual(strat.realtime_data_frame.iloc[1]['ACTIONS_MSFT'], 2)
        self.assertEqual(strat.realtime_data_frame.iloc[2]['ACTIONS_MSFT'], 0)
        self.assertEqual(strat.realtime_data_frame.iloc[3]['ACTIONS_MSFT'], -2)
